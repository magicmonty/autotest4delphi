...
procedure TGrowlNotifier.RegisterApplication;
var
...
procedure TGrowlNotifier.SendCommand(Command: TBytesStream; const Wait: Boolean = False);
var
  cmd: TSendCommand;
begin
  if Wait then
...
procedure TGrowlNotifier.SendNotification(Title, Text: string; NotificationType: TNotificationType);
var
  command: TBytesStream;
begin
  command := TBytesStream.Create();
...
procedure TGrowlNotifier.AddEndOfRequest(command: TBytesStream);
begin
  AddStringToCommand('', command);
  AddStringToCommand('', command);
end;

procedure TGrowlNotifier.AddBinaryResource(Source: TMemoryStream; ID: string; Command: TBytesStream);
var
  streamSize: Int64;
begin
...
procedure TGrowlNotifier.AddStringToCommand(Line: string; const command: TBytesStream);
var
  tmp: PAnsiChar;
  bufferSize: Cardinal;
  streamSize: Int64;
...
function TGrowlNotifier.LoadPNGResource(Id: string; const Stream: TStream): string;
var
  png: TPngImage;
begin
  Result := '';
